...
/**
 * Precompiled multiloader-paper script plugin.
 **/
@SuppressWarnings("DefaultPackage")
public class MultiloaderPaperPlugin implements org.gradle.api.Plugin<org.gradle.api.internal.project.ProjectInternal> {
    ...
    public void apply(org.gradle.api.internal.project.ProjectInternal target) {
        ...
        try {
            Class<? extends BasicScript> pluginsBlockClass = Class.forName("cp_precompiled_MultiloaderPaper").asSubclass(BasicScript.class);
            ...
            target.getPluginManager().apply("multiloader-base");
            target.getPluginManager().apply("io.papermc.paperweight.userdev");
            target.getPluginManager().apply("com.gradleup.shadow");
            target.getPluginManager().apply("xyz.jpenilla.run-paper");
            target.getPluginManager().apply("xyz.jpenilla.resource-factory-paper-convention");
            target.getPluginManager().apply("io.github.dexman545.outlet");
            target.getPluginManager().apply("com.modrinth.minotaur");


            Class<? extends BasicScript> precompiledScriptClass = Class.forName("precompiled_MultiloaderPaper").asSubclass(BasicScript.class);
  ...
  private static void assertSupportedByCurrentGradleVersion() {
      if (GradleVersion.current().getBaseVersion().compareTo(GradleVersion.version(MIN_SUPPORTED_GRADLE_VERSION)) < 0) {
          throw new RuntimeException("Precompiled Groovy script plugins built by Gradle 9.3.0 require Gradle "+MIN_SUPPORTED_GRADLE_VERSION+" or higher");
      }
  }
}
//CHECKSTYLE:ON
